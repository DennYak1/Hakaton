...
def clean_database():
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Принудительно завершаем все соединения
            cur.execute("""
            ...
            # Удаляем базу с принудительным завершением зависимостей
            ...
            with conn_db.cursor() as cur_db:
                # Создаем расширение с CASCADE перед восстановлением
...
def main():
    ...
    # Правильный порядок восстановления дампов
    dumps = [
...
#         conn.autocommit = True
#         with conn.cursor() as cur:
#             cur.execute("DROP DATABASE IF EXISTS documents")
#             cur.execute("CREATE DATABASE documents")
...
#         '--if-exists',
#         '--exit-on-error',
#         '--jobs', '1',  # Последовательное восстановление для избежания конфликтов
#         dump_file
#     ]
#     logger.info(f"Восстановление {dump_file}...")
...
#     # Правильный порядок восстановления дампов
#     dumps = [
#         '/app/WorkWithDb/filestorage.dump',  # Сначала файловое хранилище
#         '/app/WorkWithDb/cms.dump',
#         '/app/WorkWithDb/lists.dump'
#     ]
